...
export default function CollegeDetailModal({ collegeSummary, onClose, onToggleSaveCollege, isSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-3xl w-full shadow-2xl border border-slate-200 overflow-hidden max-h-[92vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
        {/* Modal Header */}
        <div className="p-4 sm:p-6 bg-gradient-to-r from-blue-900 to-indigo-900 text-white relative flex-shrink-0">
          <button
            type="button"
            onClick={onClose}
          ...
          <h3 className="text-lg sm:text-2xl font-bold pr-8 leading-tight">{fullCollege.name}</h3>

          <p className="text-xs sm:text-sm text-blue-100/90 flex items-center gap-1.5 mt-2">
            <MapPin className="w-4 h-4 text-blue-300 shrink-0" />
            <span className="line-clamp-2">{fullCollege.address?.full || `${fullCollege.address?.district || ''}, ${fullCollege.address?.state || 'Maharashtra'}`}</span>
          </p>
        </div>
            ...
            {loading ? (
              ...
              <div className="border border-slate-200 rounded-2xl overflow-hidden">
                <div className="max-h-72 overflow-y-auto overflow-x-auto">
                  <table className="min-w-[480px] w-full divide-y divide-slate-200 text-xs">
        ...
        {/* Modal Footer */}
        <div className="p-3 sm:p-4 bg-slate-50 border-t border-slate-200 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 flex-shrink-0">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 sm:flex-initial px-4 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-200/70 transition-colors cursor-pointer text-center"
            >
              Close
            </button>
            {onToggleSaveCollege && (
              <button
                ...
                className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border transition-colors cursor-pointer ${
                  isSaved
          ...
          {directionsUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 px-5 py-2.5 sm:py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold transition-all shadow-md shadow-blue-500/20 w-full sm:w-auto"
            >

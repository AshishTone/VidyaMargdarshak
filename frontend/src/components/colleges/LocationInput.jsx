  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {userLocation && (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200 max-w-full truncate">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shrink-0" />
            <span className="truncate">{userLocation.name || userLocation.district || `${userLocation.lat}, ${userLocation.lng}`}</span>
          </span>
        )}
      </div>
      ...
      <div className="pt-2 border-t border-slate-100 space-y-3">
        {userLocation?.district && (
          <div className="flex flex-wrap items-center justify-between gap-2 bg-blue-50/70 p-2.5 rounded-xl border border-blue-200/60">
            <label className="flex items-center gap-2 text-xs font-semibold text-blue-900 cursor-pointer">
              ...
              />
              <span>Show ONLY colleges located in {userLocation.district}</span>
            </label>
            {strictDistrict && (

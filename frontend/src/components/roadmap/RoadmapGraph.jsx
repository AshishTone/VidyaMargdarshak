...
function FlowCanvas({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative isolate flex flex-col rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-slate-900/5 shadow-2xl backdrop-blur transition-all overflow-hidden ${
        isFullscreen
          ? "fixed inset-0 z-50 h-screen w-screen rounded-none bg-slate-950"
          : "h-[64vh] sm:h-[74vh] w-full"
      }`}
    >
      {/* Top Toolbar with One-Career Selector */}
      <div className="z-10 flex flex-wrap items-center justify-between gap-2 sm:gap-3 border-b border-slate-200/80 bg-white/95 px-3 sm:px-5 py-2 sm:py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
        {/* Career Selector */}
        <div className="flex items-center gap-1.5 sm:gap-2 max-w-[calc(100%-80px)] sm:max-w-none">
          <div className="flex items-center gap-1.5 rounded-xl bg-blue-50 px-2 sm:px-2.5 py-1 sm:py-1.5 text-xs font-bold text-blue-700 dark:bg-blue-950 dark:text-blue-300 shrink-0">
          ...
          <div className="relative min-w-0">
            <select
              ...
              className="h-8 sm:h-9 max-w-[180px] sm:max-w-xs truncate appearance-none rounded-xl border border-slate-300 bg-white pl-2.5 sm:pl-3 pr-7 sm:pr-8 text-[11px] sm:text-xs font-extrabold text-slate-800 shadow-sm transition hover:border-blue-400 focus:border-blue-500 focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white cursor-pointer"
            >
              {careerOptions.map((c) => (
            ...
            <ChevronDown className="pointer-events-none absolute right-2 top-2 sm:right-2.5 sm:top-2.5 h-4 w-4 text-slate-400" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {activeCareerMeta && (
            <span className="hidden xl:inline-block max-w-sm truncate text-xs text-slate-500">
          ...
          <button
            ...
            className="rounded-xl border border-slate-200 bg-white p-1.5 text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 shadow-sm cursor-pointer"
            title="Fit View"
          >
      ...
      <div className="relative flex-1">
        ...
        >
          ...
          <Controls className="!m-2 sm:!m-4 !rounded-xl sm:!rounded-2xl !border !border-slate-200 !bg-white/90 !shadow-lg backdrop-blur" />
          {/* MiniMap is only shown on tablet & desktop screens to prevent mobile screen covering */}
          <MiniMap
            zoomable
            pannable
            className="hidden sm:block !m-4 !rounded-2xl !border !border-slate-200 !bg-white/90 !shadow-lg backdrop-blur"
            nodeColor={(node) => {
        ...
        {/* Clean Info Banner */}
        <div className="pointer-events-none absolute bottom-2 left-2 right-2 sm:bottom-4 sm:left-6 sm:right-auto z-10 flex items-center justify-center sm:justify-start gap-1.5 rounded-xl sm:rounded-full bg-slate-900/85 px-3 py-1.5 text-[10px] sm:text-xs font-medium text-white shadow-lg backdrop-blur text-center sm:text-left">
          <Info className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-blue-400 shrink-0" />

...
export default function CoursesPage() {
  ...
  const courses = (isTenth ? after10 : after12).filter(([name]) =>
      ...
      <SectionCard className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="section-title">
            {isTenth ? "After-10th Course Explorer" : "After-12th Course Explorer"}
          </p>
          <p className="mt-2 text-xs sm:text-sm text-slate-600">
            Showing options appropriate for your current education level only.
          </p>
        </div>
        <input
          className="w-full md:w-80 rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none focus:border-blue-500 text-sm"
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        {courses.map(([name, description]) => (
          <SectionCard key={name}>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-950">{name}</h2>
            <p className="mt-3 text-xs sm:text-sm leading-6 sm:leading-7 text-slate-600">{description}</p>
            {isTenth && name.startsWith("Higher") ? (
              <div className="mt-4 sm:mt-5 flex flex-wrap gap-2">
                {["Science", "Commerce", "Arts", "Vocational (MCVC)"].map((item) => (
                  <span
                    key={item}

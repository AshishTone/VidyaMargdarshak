    ...
    <div className="space-y-6">
      ...
      <SectionCard>
        <div className="flex items-center justify-between mb-1">
          <p className="section-title">Domain Scores</p>
          <span className="text-[11px] text-slate-400 font-medium sm:hidden">
            ↔ Swipe to view all
          </span>
        </div>
        <div className="flex h-80 items-end justify-start sm:justify-around gap-2.5 sm:gap-3 overflow-x-auto px-1 pt-6 pb-2">
          {report.result.results.map((item, index) => {
            ...
            return (
              <div
                key={item.categoryId}
                className="flex h-full min-w-[74px] sm:min-w-20 flex-1 flex-col items-center justify-end group transition-all shrink-0 sm:shrink"
              >
                <span className={`mb-2 text-xs sm:text-sm font-black ${style.scoreColor}`}>
                  {item.interestIndex.toFixed(0)}
                </span>
                <div
                  className="w-full max-w-16 sm:max-w-20 rounded-t-xl shadow-xs transition-all group-hover:brightness-95"
                  style={{
                    height: `${Math.max(item.interestIndex, 4)}%`,
                    backgroundColor: style.barColor,
                  }}
                />
                <span className="mt-2.5 sm:mt-3 text-center text-[11px] sm:text-xs font-semibold text-slate-700 dark:text-slate-300 line-clamp-2 px-0.5">
                  {labelFor(item.categoryId)}
                </span>
                <span className="mt-1 text-[10px] sm:text-[11px] text-slate-400">
        ...
        <SectionCard>
          ...
          <div className="mt-4 space-y-3">
            {structured?.recommendations.slice(0, 3).map((item, index) => (
              <div key={item.pathway} className="rounded-xl sm:rounded-2xl bg-slate-50 p-3.5 sm:p-4 border border-slate-100">
                <div className="flex justify-between gap-3">
                  <p className="text-sm sm:text-base font-bold text-slate-900">
                    {index + 1}. {item.pathway}
                  </p>
                  <p className="text-xs sm:text-sm font-bold text-blue-800 shrink-0">{item.matchScore}/100</p>
                </div>
        ...
        <SectionCard>
          ...
          {report.aiExplanation ? (
            <div className="mt-4 space-y-3 sm:space-y-4">
              <p className="text-xs sm:text-sm leading-6 sm:leading-7 text-slate-700">{report.aiExplanation.summary}</p>
              {report.aiExplanation.recommendations.slice(0, 3).map((item) => (
                <div key={item.pathway} className="rounded-xl sm:rounded-2xl bg-slate-50 p-3.5 sm:p-4 border border-slate-100">
                  <p className="text-xs sm:text-sm font-bold text-slate-900">{item.pathway}</p>
                  <p className="mt-1 text-xs text-slate-600 leading-relaxed">{item.why}</p>
                </div>
              ))}
            </div>
          ) : (
      ...
      <SectionCard>
        ...
        {canViewRoadmap ? (
          <Link to="/my-roadmap" className="mt-4 sm:mt-5 inline-block w-full sm:w-auto">
            <Button className="w-full sm:w-auto px-6 py-3 text-sm font-bold shadow-sm">View My Personalized Roadmap</Button>
          </Link>
        ) : (
          <p className="mt-3 text-xs sm:text-sm text-amber-700 font-medium">
